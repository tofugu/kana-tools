...
Lingua.kanaToRomaji = function(kana) {
  return Lingua.kanaTable[kana];
}
...
var AppBox = React.createClass({
  ...
  setActiveCharacterGroup: function(position, sentenceCharacterGroups) {
    ...
    var activeCharacterGroupRomajis = Lingua.kanaToRomaji(activeCharacterGroup);
  ...
  separateSentenceInfoCharacterGroups: function(sentence) {
    ...
    while (sentence.length > 0) {
      ...
      var potentialGroupRomajiThreeChar = Lingua.kanaToRomaji(potentialGroupThreeChar);
      var potentialGroupRomajiTwoChar = Lingua.kanaToRomaji(potentialGroupTwoChar);
  ...
  render: function() {
    return (
      ...
      <VisualFeedback characterGroups={this.state.activeSentenceCharacterGroups} inputIncorrect={this.state.isInputIncorrect} currentSentencePosition={this.state.currentSentencePosition} />
        ...
        <ErrorHistory errorHistory={this.state.errorHistory} sentences={this.state.sentences} handleSentenceIntoCharacterGroups={this.separateSentenceInfoCharacterGroups} handleClearErrorHistory={this.handleClearErrorHistory} />
      </div>
    );
  }
});
...
var VisualFeedback = React.createClass({
  render: function() {
    ...
    var characterGroupNodes = this.props.characterGroups.map(function(characterGroup, index) {
      var listItem;
      var romajiOptions = Lingua.kanaToRomaji(characterGroup).map(function(character) {
        return character.replace(' ', '{space}')
      }).join(', ');
  ...
  render: function() {
    ...
    var sentencesNodes = this.props.errorHistory.map(function(sentenceErrorInfo, index) {
      ...
      characterGroupsErrorNodes = sentenceErrorInfo.positions.map(function(position) {
        ...
        var characterGroupRomajis = Lingua.kanaToRomaji(characterGroup);
        var listItem;
        var romajiNodes;
